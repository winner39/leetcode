...
public:
    ...
    int numberOfGoodSubsets(vector<int>& nums) {
        ...
        total = add_total(count, used, boolean_count.size());
        long int index = 1;
    ...
    long int add_total(vector<int>& count, deque<int>& used, int min) {
        long int temp = 0;
        ...
        if (min >= 1) {
            for (int i = min - 1; i >= 0; i--) {
                ...
                if (count[boolean_count[i]] > 0) {
                    result += cal(count, used) % (long int)(1e9 + 7);
                }
                result += hebin(count, used, i + 1) % (long int)(1e9 + 7);
                result += add_total(count, used, i) % (long int)(1e9 + 7);
                used.pop_front();
            }
        }
        return result;
    ...
    long int hebin(vector<int>& count, deque<int>& used, int min) {
        long int temp = 0;
        long int result = 0;
        long int index_ = 0;
        ...
        if (used.size() > 1 && used[0] == boolean_count[min - 1]) {
            for (int n = 1; n < used.size(); n++) {
                ...
                if (index_ <= 30) {
                    ...
                    result += cal(count, temp_used);
                    result += add_total(count, temp_used, min - 1) % (long int)(1e9 + 7);
                }
                else {
                    break;
                }
...
void test() {
    Solution solution;
    vector<int> nums = { 10,23,8,11,29,21,27,15,8,22,24,21,28,29,2,25,1,13 };
    /*for (int i = 1; i < 21; i++) {
        nums.push_back(i);
    }*/
